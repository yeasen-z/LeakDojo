from src.skuas import GenPIDEQueryGenerator, WhiteBoxQueryLoader
...
def AtkStaticPipeline(cfg, args, 
    ...
    if args.attack == "pide":
        query_generator = GenPIDEQueryGenerator(
                            cfg.data["description"],
                            llm,
                            attack_num=args.attack_num,
                            words_used=list(processed_ids),
                            existed_entity_file=args.entity_file if args.entity_file else "attack_shop/baselines/tgtb/Random_wikitext.json",
