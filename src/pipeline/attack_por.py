...
def AtkPoRPipeline(cfg, args, lma_template=None):
    ...
    if lma_template:
        print(f"Using LMA template: {lma_template}")
        jsonl_filename = cfg.generate_exp_filename(args, f"{args.lma_method}")
    else:
    ...
    while len([data["relevance"][-1] for data in por_attacker.anchors_register.anchors_status.values() if data["relevance"][-1] > 0]) > 0:
        ...
        if lma_template:
            prompts_injected = [lma_template.format(text=batch["informations"][index]["generated_prompt"]) for index in non_completed_elements]
            print(f"Processing {len(prompts_injected)} LMA injected prompts.")
            ...
            print(f"Finished {len(prompts_injected)} LMA injected prompts.")
            ...
            for relative_index, absolute_index in enumerate(non_completed_elements):
                ...
                batch["informations"][absolute_index]["command_repetition_chunks"][0] = len(chunks)
