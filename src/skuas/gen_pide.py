...
class GenPIDEQueryGenerator(QueryGenerator):
    ...
    def __init__(self, 
        ...
        self.template = GEN_PIDE_TEMPLATES
        self.llm = llm
    ...
    def allocate_templates(self, total_questions=500):
        ...
        if self.description['type'] not in GEN_PIDE_DOMAIN_WEIGHTS:
            raise ValueError(f"Domain type '{self.description['type']}' not found in GEN_PIDE_DOMAIN_WEIGHTS")
        weights = GEN_PIDE_DOMAIN_WEIGHTS[self.description['type']]
    ...
    def generate(self) -> str:
        ...
        print(f"[INFO] Generated {len(queries)} GenPIDE attack queries, with {len(question_entity)} unique entities.")
